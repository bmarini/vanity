module Vanity
  ...
  class Metric
    ...
    def destroy!
      # This is a little nasty. We need to strip the namespace from redis.keys
      # since redis.del will at it back.
      redis.del *redis.keys(key("*")).map { |k| k.sub("#{redis.namespace}:",'') }
    end
